...
function isScheduleCreator(store,event,user=currentUser(store)){const authUserId=store?.authenticatedUserId||store?.authenticated_user_id||'';return Boolean(event?.created_by&&(event.created_by===user.id||event.created_by===authUserId))}
export function canEditEvent(store,event){if(!event)return canCreateEvents(store);const user=currentUser(store);if(!userPermission(user,'enabled'))return false;const ownsSchedule=isScheduleCreator(store,event,user);if(isSuperAdmin(store)&&hasPermission(store,'editAllEvents'))return event.schedule_source!=='admin'&&event.created_by_role!=='admin'||ownsSchedule;return isManager(store)&&user.organization_id===event.organization_id&&ownsSchedule}
export function canDeleteEvent(store,event){if(!event)return false;const user=currentUser(store);if(!userPermission(user,'enabled'))return false;const ownsSchedule=isScheduleCreator(store,event,user);if(isSuperAdmin(store)&&hasPermission(store,'deleteAllEvents'))return event.schedule_source!=='admin'&&event.created_by_role!=='admin'||ownsSchedule;return isManager(store)&&user.organization_id===event.organization_id&&ownsSchedule}
