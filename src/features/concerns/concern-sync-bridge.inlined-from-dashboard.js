...
(() => {
  ...
  function concernDedupeKey(concern = {}) {
    const contentKey = [
    ...
    return contentKey.replace(/\|/g, '') ? contentKey : `id:${concern.id || Math.random()}`;
  }

  function concernDedupeScore(concern = {}) {
  ...
  function dedupeConcernList(rows = []) {
    const byKey = new Map();
    rows.forEach((row) => {
      const concern = normalizeConcern(row);
      const key = concernDedupeKey(concern);
      const existing = byKey.get(key);
      if (!existing || concernDedupeScore(concern) >= concernDedupeScore(existing)) byKey.set(key, concern);
    });
  ...
  function syncStoreConcerns(rows) {
    ...
    s.concerns = dedupeConcernList(s.concerns);
  }
  ...
  function visibleConcerns() {
    ...
    const rows = dedupeConcernList(s?.concerns || []);
  ...
  function renderConcerns() {
    ...
    if (form && admin) form.hidden = true;
    if (!concerns.length) {
      list.innerHTML = '';
      return;
    }
    list.innerHTML = concerns.map((concern) => {
      const solved = concern.status === 'resolved';
      const action = admin && !solved
        ? `<div class="inline-actions"><button class="primary-button" type="button" data-action="concern-resolve-live" data-id="${escapeHtml(concern.id)}">Mark as Solved</button></div>`
        : '';
      return `<article class="activity-item concern-sync-card ${solved ? 'is-resolved' : 'is-pending'}" data-concern-id="${escapeHtml(concern.id)}"><header class="concern-card-header"><strong>${escapeHtml(concern.title)}</strong><span>${escapeHtml(solved ? 'Solved' : 'Pending')}</span></header>${concernDetails(concern)}${action}</article>`;
    }).join('');
  }
