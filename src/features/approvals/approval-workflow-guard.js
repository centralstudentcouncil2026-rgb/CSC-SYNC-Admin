...
(() => {
  ...
  function concernActionHtml(concern) {
    if (!isAdmin()) {
      ...
      const editable = owner && !['resolved', 'rejected'].includes(String(concern.status || '').toLowerCase()) && !pendingConcernEdit(concern.id);
      return editable
  ...
  async function reviewConcern(id, action) {
    ...
    try {
      ...
      notifyOrgFallback(
        ...
        action === 'approve'
          ? `CSC admin marked "${concern.title}" as solved.${update.admin_response ? ` Response: ${update.admin_response}` : ''}`
          : action === 'reject'
      ...
      decorateConcerns();
      alert(action === 'approve' ? 'Concern marked as solved.' : action === 'reject' ? 'Concern rejected.' : 'Concern updated.');
    } catch (error) {
  ...
  async function submitConcernEditRequest(id) {
    if (!ENABLE_CONCERN_EDIT_REQUESTS) {
      alert('Concern edit requests are not available.');
      return;
    }
    if (busy) return;
    const concern = concernById(id);
  ...
  function injectStyle() {
    ...
    style.textContent = `
      ...
      #concernsList .activity-item[data-workflow-status="resolved"] strong span{background:#dcfce7!important;color:#166534!important}
