...
(() => {
  ...
  function isRequestRecord(event) { if (event.record_type === 'blocked_time') return false; if (!event.revision_of && hasPendingRevision(event)) return false; if (event.revision_of) return event.approval_status === 'pending'; if (hasPendingAction(event)) return isLegacySameRowPending(event); return true; }
  ...
  function ensureFilters() {
    ...
    if (!filters) {
      const orgs = sortedOrganizations(store()?.organizations).map((org) => `<option value="${esc(org.id)}">${esc(org.organization_name)}</option>`).join('');
