...
async function loadRelationalStore(authenticated=false){const[profiles,organizations,calendarItems,announcements,concerns]=await Promise.all([authenticated?request('/rest/v1/profiles?select=*',{},true):Promise.resolve([]),request('/rest/v1/organizations?select=*'),request(authenticated?AUTHENTICATED_CALENDAR_ITEMS_QUERY:PUBLIC_CALENDAR_ITEMS_QUERY,{},authenticated),request('/rest/v1/announcements?select=*',{},authenticated),loadConcernsTable(authenticated)]);const items=Array.isArray(calendarItems)?calendarItems:[];const organizationNames=new Map((organizations||[]).map((item)=>[item.id,item.organization_name]));const users=(profiles||[]).map(profileToUser);const activityStatuses=(profiles||[]).map(profileToActivityStatus).filter(Boolean);return normalizeStore({version:4,currentUserId:authenticatedUserId()||'public',users,activityStatuses,pendingAccounts:(profiles||[]).filter(isPendingOrganizationProfile).map(profileToAccountRequest),organizations:organizations||[],categories:[],announcements:announcements||[],concerns:Array.isArray(concerns)?dedupeConcernRecordsForPersistence(concerns):[],blockedTimes:items.filter((item)=>item.record_type==='blocked_time').map((item)=>({...item,record_type:'blocked_time',block_source:'admin',created_by_role:'admin',requires_approval:false})),events:items.filter((item)=>item.record_type==='schedule'&&!isConferenceRoomScheduleRecord(item)).map((item)=>({...item,record_type:'schedule',organization_name:organizationNames.get(item.organization_id)||item.organization_name||'',occurrences:jsonArray(item.occurrences)}))})}
