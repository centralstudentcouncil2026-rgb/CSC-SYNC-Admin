...
function profileToUser(profile){return{...profile,username:profile.username||profile.email,profile_exists:true,database_role:profile.role||'',database_is_enabled:Boolean(profile.is_enabled),permissions:{...(profile.permissions||{}),enabled:Boolean(profile.is_enabled)}}}
...
function conferenceBookingToCalendarEvent(row={}){return{...row,record_type:'schedule',schedule_source:row.schedule_source||row.created_by_role||(row.requires_approval===false?'admin':'organization'),created_by_role:row.created_by_role||row.schedule_source||(row.requires_approval===false?'admin':'organization'),requires_approval:row.requires_approval!==false,schedule_type:row.schedule_type||'conference_room_booking',event_type:row.event_type||row.booking_type||'Conference Room Booking',venue:row.venue||'Conference Room',title:row.title||row.organization_name||'Conference Room Booking',category_id:row.category_id||'meeting',approval_status:row.approval_status||'pending',event_status:row.event_status||'planned',privacy_level:row.privacy_level||'internal',occurrences:jsonArray(row.occurrences).length?jsonArray(row.occurrences):[{date:String(row.start_time||'').slice(0,10),start_time:row.start_time||'',end_time:row.end_time||''}],updated_at:row.updated_at||row.created_at||new Date().toISOString()}}
...
export async function saveConcernRecord(concern,store=null){
  if(!session()?.access_token||!concern)return null;
  const currentStore=store||window.CONNECT_STATE?.store||{};
  const user=currentUser(currentStore);
  const now=new Date().toISOString();
  const concernId=await resolveConcernDatabaseId(concern,currentStore);
  const payload={
    status:concern.status||'pending',
    admin_response:concern.admin_response||null,
    resolved_by:uuidOrNull(concern.resolved_by)||null,
    resolved_at:concern.resolved_at||null,
    updated_at:concern.updated_at||now
  };
  const saved=await request(`/rest/v1/concerns?id=eq.${encodeURIComponent(concernId)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:body(payload)},true);
  const row=Array.isArray(saved)?saved[0]:saved;
  if(row)return row;
  throw new Error('Concern update did not match any database row. Refresh and try again.');
}
async function resolveConcernDatabaseId(concern,store={}){
  const existingId=uuidOrNull(concern.id);
  if(existingId)return existingId;
  const title=String(concern.title||'').trim();
  const organizationName=String(concern.organization_name||'').trim();
  const createdBy=uuidOrNull(concern.created_by);
  const filters=[];
  if(title)filters.push(`title=eq.${encodeURIComponent(title)}`);
  if(organizationName)filters.push(`organization_name=eq.${encodeURIComponent(organizationName)}`);
  if(createdBy)filters.push(`created_by=eq.${encodeURIComponent(createdBy)}`);
  if(filters.length){
    const rows=await request(`/rest/v1/concerns?select=id&${filters.join('&')}&order=created_at.desc&limit=1`,{},true).catch(()=>[]);
    const matchedId=uuidOrNull(Array.isArray(rows)?rows[0]?.id:null);
    if(matchedId){concern.id=matchedId;return matchedId}
  }
  const generatedId=createUuid();
  const payload=concernSnapshotRow(concern,store,generatedId);
  const saved=await request('/rest/v1/concerns',{method:'POST',headers:{Prefer:'return=representation'},body:body(payload)},true);
  const row=Array.isArray(saved)?saved[0]:saved;
  concern.id=uuidOrNull(row?.id)||generatedId;
  return concern.id;
}
async function insertConcernSnapshot(concern,store={},id=createUuid()){
  const payload=concernSnapshotRow(concern,store,id);
  const saved=await request('/rest/v1/concerns',{method:'POST',headers:{Prefer:'return=representation'},body:body(payload)},true);
  const row=Array.isArray(saved)?saved[0]:saved;
  concern.id=uuidOrNull(row?.id)||id;
  return row||payload;
}
function concernSnapshotRow(concern,store={},id=createUuid()){
  const user=currentUser(store);
  const now=new Date().toISOString();
  const organizationName=String(concern.organization_name||'').trim();
  return{
    id,
    organization_id:uuidOrNull(concern.organization_id)||uuidOrNull(user.organization_id)||null,
    organization_name:organizationName||null,
    title:String(concern.title||'').trim()||'Untitled concern',
    ...
    resolved_by:uuidOrNull(concern.resolved_by)||null,
    resolved_at:concern.resolved_at||null,
    created_by:uuidOrNull(concern.created_by)||uuidOrNull(user.id)||null,
...
async function syncOrganizationsTable(store){
  ...
  for(const organization of byName.values()){
    ...
    const saved=resolvedExistingId
      ...
      : await request('/rest/v1/organizations?on_conflict=organization_name',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:body(payload)},true);
...
async function updateOrganizationById(id,payload){
  ...
  return request('/rest/v1/organizations?on_conflict=id',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:body(payload)},true);
}
async function ensureManagedProfileOrganization(user={},payload={}){
  const organizationName=String(payload.organization_name||user.organization_name||user.organizationName||'').trim();
  if(!organizationName){payload.organization_id=null;payload.organization_name=null;return payload}
  if(!isAllowedAdminEmail(authenticatedEmail()))return payload;
  try{
    const existingRows=await request('/rest/v1/organizations?select=id,organization_name',{},true);
    const existingOrganizations=Array.isArray(existingRows)?existingRows:[];
    ...
    const saved=organizationId
      ...
      : await request('/rest/v1/organizations?on_conflict=organization_name',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:body(organizationPayload)},true);
    const savedRow=Array.isArray(saved)?saved[0]:saved;
    organizationId=savedRow?.id||organizationId;
    if(organizationId){
    ...
    throw new Error(`Could not sync organization name: ${error.message||error}`);
  }
}
...
async function verifyCalendarItemRows(rows){const ids=[...new Set((rows||[]).map((row)=>uuidOrNull(row.id)).filter(Boolean))];if(!ids.length)return;const found=await request(`/rest/v1/calendar_items?select=id&id=in.(${ids.map(encodeURIComponent).join(',')})`,{},true);const foundIds=new Set((Array.isArray(found)?found:[]).map((row)=>row.id));const missing=ids.filter((id)=>!foundIds.has(id));if(missing.length)throw new Error(`Calendar save verification failed for ${missing.join(', ')}`)}
async function verifyCalendarItemDeleted(id){const uuid=uuidOrNull(id);if(!uuid)return;const rows=await request(`/rest/v1/calendar_items?select=id&id=eq.${encodeURIComponent(uuid)}&limit=1`,{},true);if(Array.isArray(rows)&&rows.length)throw new Error(`Calendar delete verification failed for ${uuid}`)}
async function upsertCalendarItemRows(rows){if(!rows.length)return;let retryRows=rows;const strippedColumns=new Set();for(let attempt=0;attempt<12;attempt++){try{const saved=await request('/rest/v1/calendar_items?on_conflict=id',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:body(retryRows)},true);if(!Array.isArray(saved)||saved.length<retryRows.length)await verifyCalendarItemRows(retryRows);if(strippedColumns.size)console.warn('CONNECT calendar_items save skipped unsupported columns:',[...strippedColumns]);return}catch(error){const missing=missingSchemaColumn(error);if(missing&&!strippedColumns.has(missing)){strippedColumns.add(missing);retryRows=retryRows.map((row)=>{const next={...row};delete next[missing];return next});continue}await upsertCalendarItemRowsIndividually(retryRows);return}}throw new Error(`Calendar save failed after removing unsupported columns: ${[...strippedColumns].join(', ')}`)}
async function upsertCalendarItemRowsIndividually(rows){let retryRows=rows;const strippedColumns=new Set();for(let attempt=0;attempt<12;attempt++){try{for(const row of retryRows){const id=encodeURIComponent(row.id);const patched=await request(`/rest/v1/calendar_items?id=eq.${id}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:body(row)},true);if(Array.isArray(patched)&&patched.length)continue;const inserted=await request('/rest/v1/calendar_items',{method:'POST',headers:{Prefer:'return=representation'},body:body(row)},true);if(!Array.isArray(inserted)||!inserted.length)await verifyCalendarItemRows([row])}return}catch(error){const missing=missingSchemaColumn(error);if(!missing||strippedColumns.has(missing))throw error;strippedColumns.add(missing);retryRows=retryRows.map((row)=>{const next={...row};delete next[missing];return next})}}}
...
function normalizedRepeatRule(value){const text=String(value||'').trim().toLowerCase().replace(/[_-]+/g,' ');const aliases={daily:'daily','every day':'daily',weekly:'weekly','every week':'weekly',monthly:'monthly','every month':'monthly',yearly:'yearly',annually:'yearly','every year':'yearly',none:null,never:null,'does not repeat':null,'do not repeat':null,'not repeat':null};return aliases[text]||null}
...
export async function updateAccountProfile(user={},previous={}){if(!session()?.access_token||!user.id)return null;let payload=await ensureManagedProfileOrganization(user,profileUpdatePayload(user));const strippedColumns=new Set();for(let attempt=0;attempt<10;attempt++){try{for(const filter of profileUpdateFilters(user,previous)){const rows=await request(`/rest/v1/profiles?${filter}&select=*`,{method:'PATCH',headers:{Prefer:'return=representation'},body:body(payload)},true);if(Array.isArray(rows)&&rows.length)return rows[0]}return await updateAdminManagedProfile(user,previous,payload)}catch(error){const missing=missingSchemaColumn(error);if(!missing||strippedColumns.has(missing))throw error;strippedColumns.add(missing);const next={...payload};delete next[missing];payload=next}}throw new Error(`Profile update failed after removing unsupported columns: ${[...strippedColumns].join(', ')}`)}
function profileUpdatePayload(user={}){return{full_name:user.full_name||null,email:user.email||user.aup_email||null,account_type:user.account_type||null,role:user.role||null,organization_id:uuidOrNull(user.organization_id)||null,organization_name:user.organization_name||user.organizationName||null,contact_number:user.contact_number||user.phone_number||null,phone_number:user.phone_number||user.contact_number||null,is_enabled:user.permissions?.enabled!==false,permissions:user.permissions&&typeof user.permissions==='object'?user.permissions:null,updated_at:user.updated_at||new Date().toISOString()}}
function profileUpdateFilters(user={},previous={}){const candidates=[['id',previous.id],['id',user.id],['email',previous.email],['email',previous.aup_email],['email',user.email],['email',user.aup_email],['username',previous.username],['username',user.username]];const seen=new Set();return candidates.map(([column,value])=>[column,String(value||'').trim()]).filter(([,value])=>value).filter(([column,value])=>{const key=`${column}:${value.toLowerCase()}`;if(seen.has(key))return false;seen.add(key);return true}).map(([column,value])=>`${column}=eq.${encodeURIComponent(value)}`)}
async function updateAdminManagedProfile(user={},previous={},payload={}){try{const rows=await rpc('update_admin_managed_profile',{p_profile_id_text:String(previous.id||user.id||''),p_previous_email:String(previous.email||previous.aup_email||''),p_previous_username:String(previous.username||''),p_profile:payload},true);const row=Array.isArray(rows)?rows[0]:rows;if(row)return row}catch(error){if(!/function .*update_admin_managed_profile|schema cache|could not find/i.test(error.message||''))throw error}throw new Error('No matching profile row was updated. Run SQL/supabase-admin-account-edit.sql in Supabase, refresh Accounts, and try again.')}
...
export async function deleteRecord(collection,id,record={}){
  ...
  if(['events','blockedTimes','categories'].includes(collection)){
    await request(`/rest/v1/calendar_items?id=eq.${encodeURIComponent(id)}`,{method:'DELETE',headers:{Prefer:'return=representation'}},true);
    await verifyCalendarItemDeleted(id);
    return;
  }
