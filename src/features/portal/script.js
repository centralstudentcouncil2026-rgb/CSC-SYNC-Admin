...
import {
  ...
  setNotificationFilter as serviceSetNotificationFilter,
...
const TEXT_LIMITS = {
  ...
  blockedScheduleTitle: 120,
  blockedScheduleReason: 500,
  statusLabel: 80,
  search: 120
};
...
const EVENT_REQUEST_ACTION_ACCESS = new Set([
  'president@aup.edu.ph',
  'cscadviser@aup.edu.ph',
  'vicepresident@aup.edu.ph',
  'gensec@aup.edu.ph',
...
function canReviewEventRequests(store) {
  return EVENT_REQUEST_ACTION_ACCESS.has(String(accountEmail(currentUser(store)) || '').trim().toLowerCase());
}
...
function bindEvents() {
  ...
  bindSubmitActions({
    ...
    concernResponseForm: submitConcernResponseForm,
...
function scheduleAuditSnapshot(event = {}) {
  return {
    ...
    revision_of: event.revision_of || '', deleted_at: event.deleted_at || ''
  };
}
...
function renderRole() {
  ...
  document.body.classList.toggle('is-admin', isSuperAdmin(state.store));
...
function renderSidebarAccount(user = currentUser(state.store)) {
  ...
  const name = user.full_name || user.organization_name || email || 'Account';
...
function calendarEvents(monthView = isConnectedCalendarView(), viewType = state.calendar?.view.type) {
  ...
  const visibleEvents = dedupeVisibleScheduleEvents(hideReplacedOriginalSchedules(state.store.events).filter((event) => {
    ...
    return event.approval_status === 'approved' && isPublicEvent(event);
  }).filter(matchesFilters), user);
  const weekLineLayout = new Map();
  const events = visibleEvents.flatMap((event) => {
    const eventColor = scheduleEventColor(state.store, event);
    const accentColor = scheduleEventAccentColor(state.store, event);
    return monthView
  ...
  const blocks = isPublic(state.store) ? [] : state.store.blockedTimes.map((block) => {
    ...
    classNames: ['event-blocked', 'event-admin-block'],
...
const PENDING_SCHEDULE_COLOR = '#D97706';
const PENDING_SCHEDULE_ACCENT_COLOR = '#FACC15';
function isPendingSchedule(event = {}) {
  return String(event.approval_status || '').trim().toLowerCase() === 'pending';
}
function scheduleEventColor(store, event) {
  if (isConferenceRoomBooking(event)) return isPendingSchedule(event) ? PENDING_SCHEDULE_COLOR : '#2563EB';
  return isPendingSchedule(event) ? PENDING_SCHEDULE_COLOR : organizationColor(store, event);
}
function scheduleEventAccentColor(store, event) {
  if (isConferenceRoomBooking(event)) return isPendingSchedule(event) ? PENDING_SCHEDULE_ACCENT_COLOR : '#93C5FD';
  return isPendingSchedule(event) ? PENDING_SCHEDULE_ACCENT_COLOR : eventAccentColor(store, event);
}
function scheduleEventStatusClasses(event = {}) {
  const approvalStatus = String(event.approval_status || '').trim().toLowerCase();
  const eventStatus = String(event.event_status || '').trim().toLowerCase();
  return [
    approvalStatus ? `event-${approvalStatus}` : '',
    ['cancelled', 'disabled', 'completed'].includes(eventStatus) ? 'event-cancelled' : ''
  ].filter(Boolean);
}
  ...
  return occurrences.flatMap((occurrence, index) => {
    ...
    return calendarTimedSegments(occurrence).map((segment) => {
      ...
      return {
        ...
        classNames: calendarEventClassNames(semanticView, splitSegment ? null : weekPart, splitSegment ? null : weekLine, [...scheduleEventStatusClasses(event), ...(splitSegment ? ['event-week-timed-segment'] : [])]),
        extendedProps: {
          type: 'event',
          record: event,
          occurrence,
  ...
  return groupConsecutiveOccurrences(eventOccurrences(event)).flatMap((group, index) => {
    if (group.length === 1) {
      ...
      return [{
        ...
        classNames: calendarEventClassNames(calendarViewMode(viewType), null, null, [...scheduleEventStatusClasses(event), 'event-month-occurrence', 'event-month-span-single']),
    ...
    return [{
      ...
      borderColor: eventColor,
      editable: false,
      classNames: ['event-month-span', viewType === 'multiMonthYear' ? 'event-year-span' : null, 'event-month-span-multi', 'gcal-month-event', ...scheduleEventStatusClasses(event)].filter(Boolean),
...
function openEventModal(range, record = null) {
  const isBlockRecord = record?.record_type === 'blocked_time' || record?.block_source === 'admin';
  const canOpenForm = isBlockRecord ? canManageBlockRecord(record) : (record ? canEditEvent(state.store, record) : canCreateEvents(state.store));
  ...
  const savedRepeatRule = normalizedRepeatRule(record?.recurrence_type || inferRepeatRuleFromOccurrences(record, occurrences));
  ...
  const repeatUntilValue = record?.recurrence_until || record?.repeat_until || (savedRepeatRule !== 'none' ? repeatUntilFromOccurrences(occurrences) : '');
...
function updateEventEntryType() {
  ...
  if ($('eventModalTitle')) $('eventModalTitle').textContent = blockMode
    ? (state.formMode === 'edit' ? 'Edit Blocked Schedule' : 'Create Blocked Schedule')
    : (state.formMode === 'edit' ? 'Edit Schedule' : 'Create Schedule');
  document.querySelectorAll('#eventForm .schedule-only-field').forEach((node) => {
    node.hidden = blockMode;
    node.querySelectorAll('input,select,textarea').forEach((input) => {
      input.disabled = blockMode;
    });
  });
  const descriptionLabel = $('eventPublicDescription')?.closest('label');
  if (descriptionLabel) {
    descriptionLabel.querySelector('span')?.remove();
    descriptionLabel.firstChild.textContent = blockMode ? 'Block Reason' : 'Short Public Description';
  }
  if ($('eventPublicDescription')) $('eventPublicDescription').required = !blockMode;
  updateScheduleType();
}
...
function adminProfileContactCache(user = currentUser(state.store)) {
  try {
...
function inferRepeatRuleFromOccurrences(record = {}, occurrences = []) {
  const rows = [...(occurrences || [])].filter((item) => item?.start_time && item?.end_time && sameLocalDay(item.start_time, item.end_time));
  const inferred = inferRecurrenceTypeFromOccurrences(record, rows);
...
function syncEventRange(event) {
  ...
  const repeatRule = normalizedRepeatRule(event.recurrence_type);
  return {
    ...event,
    occurrences,
...
function previewBlockedScheduleSelection() {
  ...
  if (!modal?.open || eventEntryType() === 'blocked_time') return clearBlockedScheduleSelectionWarning();
...
function recurrenceDetailSummary(record, occurrences = []) {
  const rule = normalizedRepeatRule(record?.recurrence_type);
...
async function submitBlockedScheduleForm() {
  ...
  let block;
  try {
    block = readBlockedScheduleForm();
  } catch (error) {
    showToast(error.message || 'Could not read blocked schedule details.', 'error');
    return;
  }
...
function readBlockedScheduleForm() {
  const formMode = $('eventForm')?.dataset.mode || state.formMode || 'create';
  const editingId = formMode === 'edit' ? (state.editingScheduleId || $('eventForm')?.dataset.editingScheduleId || $('eventId').value) : $('eventId').value;
  const existing = state.store.blockedTimes.find((item) => item.id === editingId);
  const scheduleType = $('eventScheduleType').value;
  const startDate = $('eventDate').value;
  const endDate = scheduleType === 'multi_day' ? $('eventEndDate').value : startDate;
  const start = localIso(startDate, $('eventStart').value);
  const end = localIso(endDate, $('eventEnd').value);
  return {
    ...(existing || {}),
    id: existing?.id || (formMode === 'edit' ? editingId : '') || createId(),
    record_type: 'blocked_time',
    block_source: 'admin',
    created_by_role: 'admin',
    requires_approval: false,
    approval_status: 'approved',
    title: cleanSingleLine($('eventTitle').value),
    block_type: scheduleType === 'multi_day' ? 'multi_day' : 'single_day',
    start_time: start,
    end_time: end,
    reason: cleanMultiline($('eventPublicDescription').value),
    created_by: existing?.created_by || currentUser(state.store).id,
    created_at: existing?.created_at || new Date().toISOString(),
    updated_at: new Date().toISOString()
  };
}

function validateBlockedSchedule(block) {
  if (!block.title) return 'Complete blocked schedule title.';
  const textError = firstTextLimitError([
    [block.title, TEXT_LIMITS.blockedScheduleTitle, 'Blocked schedule title'],
    [block.reason, TEXT_LIMITS.blockedScheduleReason, 'Blocked schedule reason']
  ]);
  if (textError) return textError;
  if (!block.start_time || !block.end_time || new Date(block.start_time) >= new Date(block.end_time)) return 'Blocked schedule end date and time must be later than start.';
  return '';
}

async function saveBlockedSchedule(block) {
  const existingIndex = state.store.blockedTimes.findIndex((item) => item.id === block.id);
  const existing = existingIndex >= 0 ? state.store.blockedTimes[existingIndex] : null;
  if (existing && !canManageBlockRecord(existing)) return showToast('Only the creator can edit this blocked schedule.', 'error');
  const previous = existing ? { ...existing } : null;
  if (existingIndex >= 0) state.store.blockedTimes.splice(existingIndex, 1, block);
  else state.store.blockedTimes.push(block);
  log(existing ? 'blocked_time_updated' : 'blocked_time_created', `${existing ? 'Updated' : 'Added'} blocked schedule "${block.title}".`, block, previous);
  state.pendingCalendarDate = dateInput(block.start_time);
  const saved = await persist(existing ? 'Blocked schedule updated.' : 'Blocked schedule added.');
  if (saved) {
    closeDialog('eventModal');
    refreshCalendar();
    return true;
  }
  if (existingIndex >= 0 && previous) state.store.blockedTimes.splice(existingIndex, 1, previous);
  else state.store.blockedTimes = state.store.blockedTimes.filter((item) => item.id !== block.id);
...
function adminDatabaseWriteReady() {
  if (!isSuperAdmin(state.store)) return true;
  const user = currentUser(state.store);
  return user.profile_exists !== false
    && user.database_is_enabled !== false
    && user.database_role === 'super_admin';
}

function adminDatabaseWriteMessage() {
  return 'This admin account is not enabled as a super_admin in the Supabase profiles table, so the regular calendar cannot save. Run SQL/supabase-bootstrap-admins.sql in Supabase, refresh, and log in again.';
}
...
async function saveEvent(candidate) {
  ...
  if (!adminDatabaseWriteReady()) {
    showToast(adminDatabaseWriteMessage(), 'error');
    return false;
  }
...
function detailsActionVisibility(record) {
  ...
  if (isSuperAdmin(state.store)) {
    ...
    visibility.cancel = canEditEvent(state.store, record) && !['cancelled', 'disabled', 'completed'].includes(record.event_status);
...
function handleCalendarEventClick(info) {
  ...
  if (details?.type === 'event') return openScheduleDetails(calendarEventScheduleId(info), details);
  openDetails(details);
}
...
function handleRenderedCalendarEventPointerDown(event) {
  if (isPublic(state.store) || event.button > 0) return;
...
function cancelSelectedEvent() { const event = state.selectedDetails?.record; if (event) confirmAction(`Disable "${event.title}"?`, () => disableEvent(event)); }
...
function disableEvent(event) { updateScheduleStatusByAdmin(event, 'disabled', 'disabled', 'Schedule disabled.'); }
function cancelEvent(event) { updateScheduleStatusByAdmin(event, 'cancelled', 'cancelled', 'Event cancelled.'); }
async function updateScheduleStatusByAdmin(event, nextStatus, action, successMessage) {
  if (!event || !requirePermission(canEditEvent(state.store, event), 'You cannot update this event.')) return;
  if (event.event_status === nextStatus) return showToast(`Schedule is already ${nextStatus}.`, 'success');
  const previousValues = scheduleAuditSnapshot(event);
  event.event_status = nextStatus;
  event.updated_at = new Date().toISOString();
  const nextValues = scheduleAuditSnapshot(event);
  notifyAdminOverride(event, action);
  log(`event_${action}`, `${currentUser(state.store).full_name} ${action} "${event.title}".`, nextValues, previousValues);
  closeDialog('detailsModal');
  const saved = await persist(successMessage);
  if (!saved) await reloadStore().catch(() => {});
}
...
function deleteEventFromModal() {
  ...
  if (eventEntryType() === 'blocked_time') {
    const block = state.store.blockedTimes.find((item) => item.id === id);
    if (block) confirmAction(`Remove "${block.title || 'Blocked university period'}"?`, () => removeBlockedTime(block.id));
    return;
  }
...
async function deleteEvent(event) {
  ...
  try {
    ...
    if (isManager(state.store) && deletedEvent.created_by === currentUser(state.store).id) {
      ...
      notifyAdminOverride(deletedEvent, 'deleted');
    }
    const previousValues = scheduleAuditSnapshot(deletedEvent);
    const nextValues = { ...previousValues, event_status: 'disabled', deleted_at: new Date().toISOString(), deleted_by: currentUser(state.store).id };
    log(
      isSuperAdmin(state.store) && isOrganizationSchedule(deletedEvent) ? 'event_admin_deleted' : 'event_deleted',
...
async function submitEventReviewForm(event) {
  ...
  Object.assign(record, {
    ...
    event_status: status === 'approved' && !record.revision_of
      ? (record.event_status === 'finalized' ? 'finalized' : 'planned')
      : record.event_status,
    updated_at: now
  });
...
function announcementPreviewHtml(item) {
  ...
  return `<div class="notice"><strong>${escapeHtml(item.title)}</strong><p>${escapeHtml(item.content)}</p><p class="announcement-source">From: ${escapeHtml(source)}</p></div>`;
}
...
function openNotifications() {
  serviceSetNotificationFilter('all');
  renderNotifications();
...
function dedupeConcernsForRender(concerns = []) {
  const byKey = new Map();
  concerns.forEach((concern) => {
    if (!concern) return;
    const contentKey = [
      String(concern.created_by || '').trim().toLowerCase(),
      String(concern.organization_id || concern.organization_name || '').trim().toLowerCase(),
      String(concern.title || '').trim().toLowerCase(),
      String(concern.description || '').trim().toLowerCase(),
      String(concern.created_at || concern.updated_at || '').slice(0, 16)
    ].join('|');
    const key = contentKey.replace(/\|/g, '') ? contentKey : `id:${concern.id || Math.random()}`;
    const existing = byKey.get(key);
    if (!existing || concernRenderScore(concern) >= concernRenderScore(existing)) byKey.set(key, concern);
  });
  return [...byKey.values()].sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0));
}
function concernRenderScore(concern = {}) {
  const status = String(concern.status || '').toLowerCase();
  let score = 0;
  if (status === 'resolved') score += 8;
  if (status === 'in_review') score += 4;
  if (concern.admin_response) score += 2;
  if (concern.resolved_at) score += 2;
  score += Math.min(1, Math.max(0, new Date(concern.updated_at || concern.created_at || 0).getTime() / 8640000000000000));
  return score;
}
function renderConcerns() {
  const user = currentUser(state.store);
  const list = dedupeConcernsForRender(isSuperAdmin(state.store)
...
function openEventRequests() { if (!requirePermission(canReviewEventRequests(state.store), 'This account cannot review event requests.')) return; openDialog('eventRequestsModal'); renderEventRequests(); }
function hasSeparateRevisionRequest(event) {
  return Boolean((state.store.events || []).some((item) => item.revision_of === event.id));
}
function isLegacySameRowRequest(event) {
  return ['edit', 'remove'].includes(event.pending_action) && event.revision_status === 'pending' && !hasSeparateRevisionRequest(event);
}
function isVisibleEventRequest(event) {
  if (event.record_type === 'blocked_time') return false;
  if (event.revision_of) return event.approval_status === 'pending';
  if (['edit', 'remove'].includes(event.pending_action) || event.revision_status === 'cancel_pending' || event.event_status === 'cancellation_requested') return isLegacySameRowRequest(event);
  return true;
}
function renderEventRequests() {
  const list = $('eventRequestsList');
  if (list) {
    list.className = 'activity-list event-request-detail-grid';
    list.innerHTML = '';
...
function openBlockedTimes() { if (!requirePermission(canManageBlockedTimes(state.store), 'This account cannot manage blocked times.')) return; openEventModal(defaultRange()); }
function editBlockedTime(id) {
  ...
  openEventModal({ start: new Date(item.start_time), end: new Date(item.end_time) }, item);
}
...
const LIST_ACTION_PERMISSIONS = {
  'event-edit': { allowed: () => canReviewEventRequests(state.store), message: 'This account cannot edit event requests.' },
  'event-remove': { allowed: () => canReviewEventRequests(state.store), message: 'This account cannot remove event requests.' },
...
const LIST_ACTIONS = {
  ...
  'event-edit': editEventRequest,
  'event-remove': removeEventRequest,
  ...
  'concern-resolve': (id) => void updateConcernStatus(id, 'resolved', 'concern_resolved', 'Concern resolved.'),
  'concern-reject': (id) => void updateConcernStatus(id, 'rejected', 'concern_rejected', 'Concern rejected.'),
...
function editEventRequest(id) {
  const item = state.store.events.find((event) => event.id === id);
  if (!item) return;
  state.selectedDetails = { type: 'event', record: item };
  openScheduleEditor(state.selectedDetails);
}
...
function reviewConcern(id) {
  ...
  if (!item) return;
  state.selectedConcernResponseId = id;
  $('concernResponseTitle').textContent = item.title || 'Concern';
  $('concernResponseText').value = item.admin_response || '';
  openDialog('concernResponseModal');
  setTimeout(() => $('concernResponseText')?.focus(), 50);
}

function submitConcernResponseForm(event) {
  event.preventDefault();
  const id = state.selectedConcernResponseId;
  const item = state.store.concerns.find((concern) => concern.id === id);
  if (!item) return closeDialog('concernResponseModal');
  const adminResponse = cleanMultiline($('concernResponseText').value);
  ...
  closeDialog('concernResponseModal');
  updateConcernStatus(
    id,
    'in_review',
    'concern_responded',
...
async function updateConcernStatus(id, status, action, message, description) {
  const item = state.store.concerns.find((concern) => concern.id === id);
  if (!item) return;
  const now = new Date().toISOString();
  const user = currentUser(state.store);
  ...
  try {
    await saveConcernRecord(item, state.store);
    const latest = await loadStore();
    state.store = latest.store;
    window.CONNECT_STATE = state;
    renderAll();
    ...
    showToast(`Could not update concern in database: ${error.message}`, 'error');
  }
}
