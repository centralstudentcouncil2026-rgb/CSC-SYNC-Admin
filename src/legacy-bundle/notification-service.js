...
export function setNotificationFilter(filter = 'all') {
  activeNotificationFilter = NOTIFICATION_FILTERS.includes(filter) ? filter : 'all';
}
...
export function updateNotificationBadge() {
  updateNotificationBadgeFromNotices(currentUserNotifications());
}
...
export function renderNotifications({ onOpenTarget } = {}) {
  ...
  updateNotificationBadgeFromNotices(allNotices);
...
export function openNotificationTarget(notice = {}) {
  ...
  if (table.includes('concern')) document.getElementById('concernsButton')?.click();
  else if (table.includes('announcement')) document.getElementById('announcementsButton')?.click();
  else if (table.includes('profile')) document.getElementById('usersButton')?.click();
  else if (table.includes('conference') || type.includes('conference')) document.getElementById('conferenceRoomButton')?.click();
  else document.getElementById('eventRequestsButton')?.click();
...
function derivedScheduleNotifications(s = {}, user = currentUser()) {
  ...
  return events.flatMap((event) => {
    ...
    if (isAdmin()) {
      if (event.created_by && event.created_by === user.id) return [];
...
function updateNotificationBadgeFromNotices(notices = []) {
  const badge = document.getElementById('notificationBadge');
  if (!badge) return;
  const count = notices.filter((notice) => !notice.is_read).length;
...
async function mergeConferenceRoomBookings(payload = {}) {
  ...
  const rows = await rest('/rest/v1/conference_room_bookings?select=*&order=start_time.asc', {}, Boolean(session()?.access_token)).catch(() => []);
