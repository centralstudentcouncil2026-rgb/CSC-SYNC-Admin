  ...
  function renderConcerns() {
    ...
    list.innerHTML = concerns.map((concern) => {
      ...
      const action = admin && !solved
        ...
        : '';
      return `<article class="activity-item concern-sync-card ${solved ? 'is-resolved' : 'is-pending'}"><header class="concern-card-header"><strong>${escapeHtml(concern.title)}</strong><span>${escapeHtml(solved ? 'Solved' : 'Pending')}</span></header>${concernDetails(concern)}${action}</article>`;
    }).join('');
  }
