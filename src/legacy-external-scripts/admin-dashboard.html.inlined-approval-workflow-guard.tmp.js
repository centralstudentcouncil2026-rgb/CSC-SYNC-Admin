...
(() => {
  ...
  function concernActionHtml(concern) {
    if (!isAdmin()) {
      ...
      const editable = owner && !['approved', 'resolved', 'rejected'].includes(String(concern.status || '').toLowerCase()) && !pendingConcernEdit(concern.id);
      return editable
  ...
  function notifyOrgFallback(concern, title, message, type) {
    const s = store();
    if (!s) return;
    const recipients = (store()?.users || []).filter((user) =>
      user.id && (user.id === concern.created_by || (concern.organization_id && user.organization_id === concern.organization_id))
    );
    ...
    rows.forEach((row) => s.notifications.unshift(row));
  }

  function notifyAdminsFallback(referenceId, title, message, type) {
    const s = store();
    if (!s) return;
    const rows = (s.users || [])
      .filter((user) => user.id && isAdmin(user))
      .map((user) => ({
        notification_id: createId(),
        user_id: user.id,
        notification_type: type,
  ...
  async function reviewConcern(id, action) {
    ...
    try {
      ...
      notifyOrgFallback(
        concern,
        action === 'approve' ? 'Concern Approved' : action === 'reject' ? 'Concern Rejected' : 'Concern Status Updated',
        action === 'approve'
        ...
        action === 'approve' ? 'concern_approved' : action === 'reject' ? 'concern_rejected' : 'concern_update'
      );
      decorateConcerns();
      alert(action === 'approve' ? 'Concern approved.' : action === 'reject' ? 'Concern rejected.' : 'Concern updated.');
    } catch (error) {
