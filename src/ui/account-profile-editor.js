(() => {
  ...
  function updateLocalProfile(profile) {
    ...
    if (name) {
      name.textContent = profile.full_name || email || 'Account';
      name.title = email ? `${profile.full_name || 'Account'} (${email})` : profile.full_name || 'Account';
    }
    if (avatar) avatar.textContent = initials(profile.full_name || email || 'A');
  }
